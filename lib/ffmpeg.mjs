...
// Mix stems over an assembled cut: music bed (ducked under VO) + optional SFX
// accents delayed to their beat offsets. Video stream is copied untouched.
export function mixStems(videoPath, musicPath, sfxEntries, outPath) {
  ...
  const mix = ['[0:a]'];
  let idx = 1;
  if (musicPath) { parts.push(`[${idx}:a]volume=0.22[mus]`); mix.push('[mus]'); idx++; }
  sfxEntries.forEach((s, i) => {
    const d = Math.max(0, Math.round(s.atSec * 1000));
    parts.push(`[${idx}:a]adelay=${d}|${d},volume=0.5[fx${i}]`);
    mix.push(`[fx${i}]`); idx++;
  });
  parts.push(`${mix.join('')}amix=inputs=${mix.length}:duration=first:normalize=0[aout]`);
  run('ffmpeg', [
    '-y', ...inputs,
