...
import { callMultimodalStructured, callStructured } from './provider-failover.mjs';
...
async function call({ system, content, schema, maxTokens = 4000, _retry = true }) {
  // Text-only council work uses the subscription-first frontier chain. The
  // frame-review path remains multimodal and uses its explicitly configured
  // image-capable route because local subscription CLIs cannot receive the
  // generated frame bytes as structured image input.
  if (Array.isArray(content)) return callMultimodalStructured({ system, content, schema, maxTokens });
  const result = await callStructured({ system, content, schema, maxTokens });
  return { out: result.out, costUsd: result.costUsd, provider: result.provider, model: result.model };
}
