...
ElevenLabs-native production pipeline: a Markdown voiceover script in `input/` becomes a finished short-form video with narration, AI b-roll, composed music, sound design, captions, and optional multilingual dubs. Seven stages orchestrated by `pipeline.mjs`, with every API call cost-logged to `output/run-manifest.json`. Built as a forward-deployed reference workflow; the manifest IS the case-study evidence. Status: v0.3, end-to-end live.
