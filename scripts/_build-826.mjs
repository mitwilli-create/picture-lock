// $8.26 re-render builder (2026-07-16): parameterized copy of _build-e5.mjs
// (the v13 builder state) reading a take set produced by _rerender-826-audio.mjs.
// Only the input/output names and cache dirs differ from _build-e5.mjs; the
// visual grammar, composites, SFX map, mix and loudness chain are unchanged.
// Usage: SET=826a node scripts/_build-826.mjs [--scenes-only|--assemble-only]
...
const SC = `${E}/scenes-${SET}`;
const TMP = `.cache/takes-${SET}`;
...
const B = JSON.parse(readFileSync(`${E}/beats-${SET}.json`, 'utf8'));
const at = id => B.beats.find(b => b.id === id).start;
const words = JSON.parse(readFileSync(`${E}/narration-words-${SET}.json`, 'utf8')).words.filter(w => w.type === 'word' && /[a-z0-9]/i.test(w.text));
const wordAt = (txt, after = 0) => words.find(w => w.start >= after && w.text.toLowerCase().replace(/[^a-z]/g, '') === txt)?.start ?? null;

const lead = 0.45;
const TAIL = 3.5;
const END = B.total + TAIL;
...
ff(['-ss', String(rolodexTrim), '-i', `${E}/anim/rolodex.mp4`, '-i', `${SC}/problem.mp4`,
...
const navT = +((nrWord - 1.2) - at('different')).toFixed(2);
const nrT = +((at('source') - 0.4) - (nrWord - 1.2)).toFixed(2);
const XF = 0.25;
...
await renderMograph({ template: 'explainer', seconds: ES_D,
  ...
  outPath: `${SC}/esbanner.mp4`, log: m => process.stdout.write('\r' + m + '   ') });
...
ff(['-i', `${SC}/steps.mp4`, '-i', `${CART}/p1-feed.mp4`, '-i', `${CART}/p2-generate.mp4`, '-i', `${CART}/p3-receipt.mp4`,
...
for (const mask of [false, true]) {
  await renderMograph({ template: 'explainer', seconds: mDur,
    ...
    outPath: `${SC}/mcard${mask ? '-mask' : ''}.mp4`, log: m => process.stdout.write('\r' + m + '   ') });
}
...
await renderMograph({ template: 'explainer', seconds: SYN_D,
  ...
  outPath: `${SC}/synbanner.mp4`, log: m => process.stdout.write('\r' + m + '   ') });
console.log('\nscene synbanner', SYN_D + 's');
ff(['-stream_loop', '2', '-i', `${E}/anim/voicewave.mp4`, '-i', `${SC}/voice.mp4`,
  '-f', 'lavfi', '-t', '0.19', '-i', 'color=c=0x9A4C42:s=1920x1080:r=30',
  '-i', `${SC}/mcard.mp4`, '-i', `${SC}/mcard-mask.mp4`, '-i', `${SC}/synbanner.mp4`,
...
push(`${SC}/hook.mp4`, 0, SCENES[0].dur);
push(`${TMP}/problem-comp.mp4`, 0, SCENES[1].dur);
push(`${TMP}/nav-pair.mp4`, 0, +(navT + nrT).toFixed(2));
push(`${SC}/newsroom.mp4`, 0, SCENES[2].dur);
push(`${TMP}/projects-v5.mp4`, 0, pubDur);
push(`${TMP}/steps-comp.mp4`, 0, SCENES[3].dur);
push(`${SC}/proof.mp4`, 0, SCENES[4].dur);
push(`${TMP}/voice-comp.mp4`, 0, SCENES[5].dur, { fadeout: 0.2 });
push('output/takes/e2/hiring-v6/handshake.mp4', 2.0, 2.6, { fadein: 0.25 });
push(`${SC}/cta.mp4`, 2.6, SCENES[6].dur - 2.6);
...
ff(['-i', `${TMP}/video.mp4`, '-i', `${E}/narration-${SET}.mp3`, '-map', '0:v', '-map', '1:a',
  '-af', 'apad', '-shortest', '-c:v', 'copy', '-c:a', 'aac', '-b:a', '192k', `${TMP}/va.mp4`]);

ff(['-i', `${E}/score-bed-${SET}.mp3`, '-af', 'dynaudnorm=f=700:g=21:m=6:p=0.6', '-c:a', 'libmp3lame', '-b:a', '192k', `${TMP}/bed-dyn.mp3`]);
...
const sfxMap = [
  ...
  { path: SFX('whoosh.mp3'),    atSec: at('different') + navT },
  ...
  { path: SFX('stamp-lo.mp3'),  atSec: at('mitchell') + 0.42 },
...
ff(['-i', `${TMP}/mix.mp4`, '-af',
  ...
  '-c:v', 'copy', '-c:a', 'aac', '-b:a', '192k', '-movflags', '+faststart', `output/takes/picture-lock-explainer-${SET}.mp4`]);

// captions from this take's words (fresh)
const cues = []; let cur = [];
...
writeFileSync(`output/takes/picture-lock-explainer-${SET}.srt`, cues.map((c, i) =>
  `${i + 1}\n${ts(c[0].start)} --> ${ts(c.at(-1).end + 0.15)}\n${c.map(x => x.text).join(' ')}\n`).join('\n'));
console.log(`picture-lock-explainer-${SET}.mp4 done; input`, m.input_i, '→ -16.5 LUFS');
