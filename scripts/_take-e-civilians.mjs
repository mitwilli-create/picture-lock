...
// attention conditions WATCH the cut (Gemini multimodal) and log, first-person,
// exactly when they drift, lose the thread, or get bored. Opus then extracts
...
const r = await fetch('https://api.anthropic.com/v1/messages', {
  ...
  body: JSON.stringify({
    ...
    messages: [{ role: 'user', content: reactions.map(x => `## ${x.id}\n${x.text}`).join('\n\n---\n\n') }],
  }),
  signal: AbortSignal.timeout(300_000),
});
if (!r.ok) throw new Error(`opus → ${r.status}`);
const j = await r.json();
const lessons = j.content.map(c => c.text ?? '').join('');
