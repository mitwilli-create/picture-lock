...
// accents, -14 LUFS / -1.5 dBTP master.  Usage: node scripts/_bundle-a-remix3.mjs
...
const TREAT = {
  ...
  16: null, 19: { vol: 0.45, lp: 3500 }, 20: { vol: 0.45, lp: 3500 },
};
const entries = [];
...
F.push(`[0:a]asplit=2[voice][vkey]`);
...
// beds
F.push(`[1:a]volume=0.5,afade=t=in:st=0:d=1.2,afade=t=out:st=${fadeOutAt}:d=2.8[mus]`);
F.push(`[2:a]volume=0.34,lowpass=f=8500,afade=t=in:st=0:d=0.8,afade=t=out:st=${fadeOutAt}:d=2.8[ambb]`);
...
F.push(`[beds][vkey]sidechaincompress=threshold=0.02:ratio=4:attack=25:release=650:makeup=1[bedsd]`);
// final: voice on top, master to -14 LUFS / -1.5 dBTP
F.push(`[voice][bedsd]amix=inputs=2:duration=first:normalize=0,loudnorm=I=-14:TP=-1.5:LRA=11[aout]`);

const out = join(ROOT, 'output', 'cover-remix3.mp4');
