...
Subject: an investigator's evidence board rendered as an engraved technical diagram. A large wall board in thin bone-cream linework holds pinned rectangular documents, small photographs, and index cards arranged loosely across the frame, each pinned with a small circular pin. Taut straight string lines run from pin to pin, several of them converging toward one central larger case file folder pinned at the middle-right of the board, its folder tab rendered as the single solid oxblood accent fill. Generous dark negative space at the edges; the board does not fill the whole frame.`,
    motion: `Subtle cinemagraph, camera locked off, composition and framing completely unchanged from the still. Animate exactly one element: a faint gleam of light travels once along one string line from an outer pinned document toward the central oxblood case file, then fades. Nothing else moves; documents, pins, and board stay completely still. No strobing, no flashing, no camera motion.`,
  },
};
